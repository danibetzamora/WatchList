...
@Component({
  ...
  templateUrl: './films-list-view.component.html',
  styleUrls: ['./films-list-view.component.css']
})
  ...
  public getListFilms(){
    ...
    this.userService.getDocument(this.userId).subscribe(
      (res: any) => {
        ...
        if (Array.isArray(this.user.films_watched_list)) {
          const uniqueFilmIds = new Set<string>();

          for (const filmID of this.user.films_watched_list) {
